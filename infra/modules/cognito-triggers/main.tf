...
# Create Auth Challenge (generate OTP, send via SNS)
  ...
  source {
    content  = <<-JS
      ...
      exports.handler = async (event) => {
        ...
        console.log("[OTP]", phone.slice(0, 6) + "****", "code:", otp);

        try {
          await sns.send(new PublishCommand({
            PhoneNumber: phone,
            Message: "Your Area Code code is: " + otp,
            MessageAttributes: {
              "AWS.SNS.SMS.SMSType": {
                DataType: "String",
          ...
          console.log("[OTP] SMS sent to", phone.slice(0, 6) + "****");
        } catch (err) {
...
# Verify Auth Challenge (check OTP)
