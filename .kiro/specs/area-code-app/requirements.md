...
4. WHEN a customer reaches the consumer sign-up entry point on the consumer web app (`areacode.co.za`), THE Auth_System SHALL route directly to `/signup` (the consumer signup form) — no "I'm a customer / I'm a business" hard-fork is shown to consumers, because the business portal lives on a separate subdomain (`business.areacode.co.za`) reached via direct link from sales onboarding, and surfacing a business path on the consumer surface leaks that the two portals are part of one platform.
