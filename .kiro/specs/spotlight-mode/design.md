...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | Enter zoom: fly to `MAP_ARRIVAL_ZOOM` or preserve zoom?                                                                                                  | Preserve the user's zoom; pan-only framing via the existing Active_Venue fly-to.                                                                                    | Matches the map-carousel "preserve the user's zoom" contract; zoom-out is the release gesture, so changing zoom on enter would move the exit goalposts.                                                                                                                         |
...
| D4  | Long-press on the already-active venue moves the camera?                                                                                                 | No camera move.                                                                                                                                                     | `activeVenueId` does not change, so the existing fly-to effect (keyed on change) does not fire; adding a forced re-frame would be a second camera path and would punish a user who deliberately repositioned. This falls out of the code structurally.                          |
...
| D11 | Where is the entry zoom recorded?                                                                                                                        | A `MapScreen` ref, captured on the `spotlightVenueId` null-to-id transition.                                                                                        | The store stays pure and map-free; the map read belongs to the screen that owns the map instance.                                                                                                                                                                               |
...
- Expose `spotlightVenueId` and `enterSpotlight` (store passthrough) on the
  result (R1.7).

## Camera (no new code path)

Enter reuses the existing "fly to Active_Venue on change" effect: because
`enterSpotlight` sets `activeVenueId`, a spotlight on a non-active venue
flies exactly like a card tap, preserving the user's zoom (the effect omits
`zoom` above `MIN_MARKER_ZOOM`). Spotlight on the already-active venue does
not move the camera (D4). Exit never moves the camera; the Recenter exit's
own fly-to proceeds as normal.
...
- `PeekCarousel` composes the long-press handlers into its existing
  ...
  `[data-venue-card]` target. `onCardSelect` checks `didFire()` before
  running `selectVenue`, so a hold never doubles as a select. The swipe
  path needs no guard: a fired hold implies sub-tolerance movement, which
  `classifyDrag` already reads as `indeterminate`.
