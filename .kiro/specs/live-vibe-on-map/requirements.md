...
### Requirement 8: Archetype glyph as the live-map node

**User Story:** As a consumer, I want each venue marker to _be_ the archetype glyph, so that I can read the map's vibe without opening every venue.

#### Acceptance Criteria

1. THE Node SHALL render an Archetype_Glyph as the marker itself. The previous "category-coloured core circle with a glyph overlaid" treatment SHALL NOT be rendered. The marker container SHALL still own the Pulse_State halo (R8.5) and the popping ripple, both of which sit behind the glyph silhouette and do not occlude it.
2. THE Archetype_Glyph SHALL use the Archetype's `iconId` to look up an SVG glyph from a new shared glyph registry.
3. WHILE the Pulse_State is `dormant`, THE Archetype_Glyph SHALL render at 50% ± 5% opacity.
4. WHILE the Pulse_State is `quiet`, `active`, `buzzing`, or `popping`, THE Archetype_Glyph SHALL render at 100% opacity.
5. THE Archetype_Glyph SHALL share the marker wrapper's per-state animation (breathe / pulse) so that its scale curve stays within 16ms of the halo's scale curve.
...
9. THE Archetype_Glyph SHALL render at no smaller than 8px AND, at every Pulse_State, SHALL render with two stacked SVG layers: a stroked outline pass underneath in the colour returned by `dynamicContrastForCategory(category)`, and a fill pass on top in the venue's category colour from `getCategoryColour(category)`. The contrast ratio between the silhouette colour (fill) and the outline colour SHALL be at least 3:1, so the silhouette reads on top of its own outline regardless of basemap.

   _Rationale._ Both layers render at 1.0 opacity inside the SVG; the wrapper's CSS `opacity` for R8.3 / R8.4 scales them together against the basemap, so the silhouette / outline pair stays in lockstep at every Pulse_State and the gamma-space compositing footgun from the previous "fill on top of node core" design no longer applies. R8.9's 3:1 floor therefore holds across all five Pulse_States in this layout.
...
10. WHEN an Archetype_Glyph is rendered, THE silhouette colour (the venue's category hex from `getCategoryColour(category)`) and the outline colour (from `dynamicContrastForCategory(category)`) SHALL maintain the R8.9 contrast ratio of at least 3:1 across every (archetype × Pulse_State × category) combination from the live catalog at any supported display zoom. The property generator SHALL enumerate the cross-product (currently 15 archetypes × 5 pulse states × 6 categories = 450 cells) and assert the silhouette/outline contrast at the smallest supported glyph size.
...
4. THE Map_Sidebar SHALL no longer render the legacy permanent City_Pulse glass card under any flag state. The City_Pulse readout SHALL appear only as the City_Pulse_Toast on the map tab. Nodes SHALL always render their Archetype_Glyph (R8.1) using the live archetype id when present, otherwise the node's `defaultArchetypeId`, otherwise `archetype-eclectic`. WHILE `live_vibe_on_map` is `false`, the live `node:archetype_change` socket subscription (R11) SHALL NOT mount, so the cached `archetypeIds` value updates only on the initial nodes payload — the glyph renders, but it stays on the venue's default until the flag flips.
...
| Archetype_Glyph silhouette ≥ 3:1                          | every (archetype × Pulse_State × category) combination in the live catalog at the smallest supported glyph size                                                                                        | Glyph is rendered with the silhouette in the category colour and the outline in `dynamicContrastForCategory(category)` |
