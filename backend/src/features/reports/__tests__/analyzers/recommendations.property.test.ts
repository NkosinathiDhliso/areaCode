...
const hourlyDistributionArb: fc.Arbitrary<Record<number, number>> = fc.tuple(
  ...Array.from({ length: 24 }, () => fc.integer({ min: 0, max: 100 })),
).map((counts) => {
...
const dailyDistributionArb: fc.Arbitrary<Record<string, number>> = fc.tuple(
  ...Array.from({ length: 7 }, () => fc.integer({ min: 0, max: 200 })),
).map((counts) => {
...
const peakHoursArb: fc.Arbitrary<PeakHoursResult> = fc.tuple(
  hourlyDistributionArb,
  dailyDistributionArb,
...
describe('Feature: venue-intelligence-reports, Property 10: Recommendation Generation Bounds and Conditions', () => {
  ...
  it('generates retention alert when repeat rate drops > 10 percentage points', () => {
    ...
    fc.assert(
      fc.property(
        reportSectionsArb,
        fc.double({ min: 11, max: 80, noNaN: true }),
        (baseReport, drop) => {
