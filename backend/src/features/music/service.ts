...
const ARCHETYPES: ArchetypeDef[] = [
  {
    ...
    thresholds: { energy: 0.7, cultural_rootedness: 0.6, edge: 0.4 },
    priority: 15,
  },
  {
    id: 'archetype-conscious-creative',
    name: 'The Conscious Creative',
    thresholds: { spirituality: 0.4, edge: 0.4, sophistication: 0.4 },
    priority: 14,
  },
  {
    id: 'archetype-township-royal',
    name: 'The Township Royal',
    thresholds: { cultural_rootedness: 0.7, energy: 0.6, edge: 0.4 },
    priority: 13,
  },
  {
    id: 'archetype-sacred-rebel',
    name: 'The Sacred Rebel',
    thresholds: { spirituality: 0.6, edge: 0.6 },
    priority: 12,
  },
  { id: 'archetype-firecracker', name: 'The Firecracker', thresholds: { energy: 0.7, edge: 0.6 }, priority: 11 },
  {
    ...
    thresholds: { energy: 0.7, cultural_rootedness: 0.6 },
    priority: 10,
  },
  {
    id: 'archetype-midnight-philosopher',
    name: 'The Midnight Philosopher',
    thresholds: { sophistication: 0.7, spirituality: 0.4 },
    priority: 9,
  },
  {
    id: 'archetype-street-poet',
    name: 'The Street Poet',
    thresholds: { edge: 0.6, cultural_rootedness: 0.4 },
    priority: 8,
  },
  {
    id: 'archetype-soul-wanderer',
    name: 'The Soul Wanderer',
    thresholds: { spirituality: 0.6, sophistication: 0.6 },
    priority: 7,
  },
  {
    id: 'archetype-vibe-architect',
    name: 'The Vibe Architect',
    thresholds: { sophistication: 0.6, energy: 0.4 },
    priority: 6,
  },
  ...
  { id: 'archetype-groove-seeker', name: 'The Groove Seeker', thresholds: { energy: 0.7 }, priority: 4 },
  {
    ...
    thresholds: { cultural_rootedness: 0.7 },
    priority: 3,
  },
...
function resolveArchetypeId(scores: Record<string, number> | null): string {
  ...
  const sorted = ARCHETYPES.filter((a) => a.name !== 'The Eclectic' && a.name !== 'The Uncharted').sort(
...
export async function handleSpotifyCallback(code: string, state: string): Promise<string> {
  ...
  try {
    ...
    // The apex `areacode.co.za` is not routed through Amplify (only `www.` is),
    // so redirecting there would 404. Normalize to www so the OAuth success
    // state actually reaches the frontend and updates the Connected UI.
    if (frontendBase === 'https://areacode.co.za' || frontendBase === 'http://areacode.co.za') {
      frontendBase = 'https://www.areacode.co.za'
    }
...
export async function getCrowdVibe(nodeId: string): Promise<CrowdVibeSnapshot> {
  ...
  const aggregateDimensionScores =
    dimCount > 0
      ? (Object.fromEntries(
          DIMENSIONS.map((d) => [d, Math.round((dimSums[d]! / dimCount) * 1000) / 1000]),
        ) as unknown as DimensionScoreVector)
      : null
