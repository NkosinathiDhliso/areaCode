...
export async function profileRoutes(app: FastifyInstance) {
  ...
  app.get('/v1/users/me/tier-progress', { preHandler: [requireAuth('consumer')] }, async (request) => {
    ...
    const totalCheckIns = ((profile as unknown as Record<string, unknown>).totalCheckIns as number) ?? 0
  ...
  app.get('/v1/users/me/streak', { preHandler: [requireAuth('consumer')] }, async (request) => {
    const auth = getAuth(request)
    const profile = (await service.getUserProfile(auth.cognitoSub)) as unknown as Record<string, unknown>
    ...
    if (streakCount > 0) {
      ...
      if (history.items.length > 0) {
        const lastCheckIn = new Date((history.items[0] as unknown as { checkedInAt: string }).checkedInAt)
