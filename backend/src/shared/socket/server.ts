...
/**
 * Returns the Socket.io server if initialised, or null if not (e.g., in Lambda
 * environments where socket.io isn't attached). Use this for fire-and-forget
 * broadcasts that should silently no-op when the socket server isn't available.
 */
export function tryGetIO(): TypedServer | null {
  return io
}
