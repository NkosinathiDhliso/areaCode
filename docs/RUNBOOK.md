...
| Cognito email pool cutover (`business-v2` / `staff-v2`, spec `cognito-email-pool-cutover`) | Replace the phone-username prod business/staff pools with email-username V2 pools, then switch Lambda env and Amplify `VITE_COGNITO_*` vars to the V2 values | prod        | PENDING                           | pending                       | Not run. Old_Pools: business `us-east-1_ToRjJQAGY`, staff `us-east-1_IgGAzUdON`. V2_Pools IDs, cutover date, and verification outcome are pending the founder-run cutover. See the "Cognito email pool cutover record" below.                                                                                                                                             |
...
### Cognito email pool cutover record

Founder fills this in at cutover time. Status: pending. Spec:
`cognito-email-pool-cutover`. The Old_Pools IDs are the rollback pointer while
the old pools exist and the historical record after deletion (deletion is a
separate gated step; see the spec's task 8). Do not fabricate the V2 IDs, the
date, or the outcome; fill each row from the Terraform outputs and the live
verification when the cutover runs.

| Field                | Value                               |
| -------------------- | ----------------------------------- |
| Cutover date         | pending                             |
| Environments         | prod (business + staff pools)       |
| Old business pool ID | us-east-1_ToRjJQAGY                 |
| Old staff pool ID    | us-east-1_IgGAzUdON                 |
| V2 business pool ID  | pending (Terraform output)          |
| V2 staff pool ID     | pending (Terraform output)          |
| Verification outcome | pending (Requirement 4 live checks) |
